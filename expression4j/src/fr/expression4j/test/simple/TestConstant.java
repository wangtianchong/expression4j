...
public class TestConstant extends TestCase {
	...
	public void testConstantPi() {
		...
		try {
			...
			assertEquals("Cannot evaluate pi constant",3.14,expression.evaluate(null).getRealValue(mc).doubleValue(),0.01);
		}
	...
	public void testConstantE() {
		...
		try {
			Expression expression = ExpressionFactory.createExpression(expressionString);
			assertEquals("Cannot evaluate e constant",2.71,expression.evaluate(null).getRealValue(mc).doubleValue(),0.01);
		}
		catch (ParsingException pe) {
			fail("Cannot parse simple expression. " + pe);
		}
	...
	public void testConstantEWithReal() {
		...
		try {
			Expression expression = ExpressionFactory.createExpression(expressionString);
			assertEquals("Cannot evaluate expression with e constant.",0.006,expression.evaluate(null).getRealValue(mc).doubleValue(),0.001);
		}
		catch (ParsingException pe) {
			fail("Cannot parse simple expression. " + pe);
		}
	...
	public void testCustomConstant() {
		...
		MathematicalElement me = NumberFactory.createReal(new BigDecimal(2.5));
		...
		try {
			...
			assertEquals("Cannot evaluate expression with custom foo constant.",5,expression.evaluate(null).getRealValue(mc).doubleValue(),0);
		}
		catch (ParsingException pe) {
			fail("Cannot parse simple expression. " + pe);
		}
	...
	public void testCustomComplexConstant() {
		Catalog catalog = ExpressionFactory.createCatalog("Catalog",mc);
		MathematicalElement me = NumberFactory.createComplex(new BigDecimal(2.5),new BigDecimal(3));
		catalog.addConstant("fooConstant",me);
		
		String expressionString = "f()=2*fooConstant";
		try {
			Expression expression = ExpressionFactory.createExpression(expressionString,catalog);
			assertEquals("Cannot evaluate real value for expression with custom foo constant.",5,expression.evaluate(null).getRealValue(mc).doubleValue(),0);
			assertEquals("Cannot evaluate complex value for expression with custom foo constant.",6,expression.evaluate(null).getComplexValue(mc).doubleValue(),0);
		}
		catch (ParsingException pe) {
			fail("Cannot parse simple expression. " + pe);
		}
