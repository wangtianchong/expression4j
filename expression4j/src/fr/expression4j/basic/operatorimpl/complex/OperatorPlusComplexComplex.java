...
public class OperatorPlusComplexComplex implements OperatorImpl {
	...
	public MathematicalElement compute(MathContext mc,MathematicalElement leftElement,
		...
		try {
			...
			BigDecimal realValue = leftElement.getRealValue(mc).add(rightElement.getRealValue(mc),mc);
			BigDecimal complexValue = leftElement.getComplexValue(mc).add(rightElement.getComplexValue(mc),mc);
