...
public class TestFunction extends TestCase {
	...
	public void testSimpleFunction() {
		...
		try {
			...
			assertEquals("Invalid evaluation value.",11.0,expression2.evaluate(parameters).getRealValue(mc).doubleValue(),0);
		}
	...
	public void testPredefinedFunction() {
		...
		try {
			...
			assertEquals("Invalid evaluation value.",10.0,expression.evaluate(parameters).getRealValue(mc).doubleValue(),0);
		}
		catch (ParsingException pe) {
			fail("Cannot parse simple expression. " + pe);
		}
	...
	public void testFunctionWithGivenCatalog() {
		...
		try {
			...
			assertEquals("Invalid evaluation value.",20.0,expression2.evaluate(parameters).getRealValue(mc).doubleValue(),0);
		}
		catch (ParsingException pe) {
			fail("Cannot parse simple expression. " + pe);
		}
