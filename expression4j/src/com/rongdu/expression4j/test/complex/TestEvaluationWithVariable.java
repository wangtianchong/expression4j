...
package com.rongdu.expression4j.test.complex;
...
public class TestEvaluationWithVariable extends TestCase {
	...
	public void testEvaluationSmallVariable() {
		...
		try {
			...
			parameters.addParameter("x",NumberFactory.createReal(new BigDecimal(5)));
			
			assertEquals("Invalid evaluation value.",11.0,expression.evaluate(parameters).getRealValue(mc).doubleValue(),0);
		}
	...
	public void testEvaluationBigVariable() {
		...
		try {
			...
			Parameters parameters = ExpressionFactory.createParameters();
			parameters.addParameter("ABCDEFGHIJKLMNO45",NumberFactory.createReal(new BigDecimal(5)));
	...
	public void testEvaluationVariableMoreThanOne() {
		...
		try {
			...
			Parameters parameters = ExpressionFactory.createParameters();
			parameters.addParameter("x",NumberFactory.createReal(new BigDecimal(4)));
	...
	public void testTranslateVariableName() {
		...
		try {
			...
			Parameters parameters = ExpressionFactory.createParameters();
			parameters.addParameter("y",NumberFactory.createReal(new BigDecimal(4)));
			
			assertEquals("Invalid evaluation value.",22.0,expression2.evaluate(parameters).getRealValue(mc).doubleValue(),0);
		}
		catch (ParsingException pe) {
			fail("Cannot parse simple expression. " + pe);
		}
