...
public class TestAddition extends TestCase {
	...
	public void testAddition1() {
		...
		try {
			...
			assertEquals(3.0,expression.evaluate(null).getRealValue(mc).doubleValue(),0);
		}
	...
	public void testAddition2() {
		...
		try {
			Expression expression = ExpressionFactory.createExpression(expressionString);
			assertEquals(6.0,expression.evaluate(null).getRealValue(mc).doubleValue(),0);
		}
		catch (ParsingException pe) {
			fail("Cannot parse simple expression. " + pe);
		}
	...
	public void testAddition3() {
		...
		try {
			Expression expression = ExpressionFactory.createExpression(expressionString);
			assertEquals(10.0,expression.evaluate(null).getRealValue(mc).doubleValue(),0);
		}
		catch (ParsingException pe) {
			fail("Cannot parse simple expression. " + pe);
		}
	...
	public void testAdditionWithComplex() {
		...
		try {
			...
			mathematicalElement = NumberFactory.createReal(new BigDecimal(4));
			Parameters parameters = ExpressionFactory.createParameters();
			parameters.addParameter("x",mathematicalElement);
			
			MathematicalElement result = expression.evaluate(parameters);
			assertEquals("Invalid real value.",12,result.getRealValue(mc).doubleValue(),0);
			assertEquals("Invalid complex value.",-2,result.getComplexValue(mc).doubleValue(),0);
			
			mathematicalElement = NumberFactory.createComplex(new BigDecimal(4),new BigDecimal(5));
			parameters.addParameter("x",mathematicalElement);
			
			result = expression.evaluate(parameters);
			assertEquals("Invalid real value.",12,result.getRealValue(mc).doubleValue(),0);
			assertEquals("Invalud complex value.",8,result.getComplexValue(mc).doubleValue(),0);
		}
		catch (ParsingException pe) {
