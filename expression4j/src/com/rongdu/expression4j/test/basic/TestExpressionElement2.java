...
import com.rongdu.expression4j.basic.impl.ParenthesisExpressionElement;
import com.rongdu.expression4j.basic.impl.RealExpressionElement;
import com.rongdu.expression4j.basic.impl.RealImpl;
import com.rongdu.expression4j.basic.impl.RealSimpleExpressionElement;
import com.rongdu.expression4j.basic.impl.SignedExpressionExpressionElement;
import com.rongdu.expression4j.basic.impl.SimpleExpressionExpressionElement;
import com.rongdu.expression4j.basic.impl.UnaryOperatorExpressionElement;
	...
	public void testRealExpressionElement() {
		try {
			...
			if (expressionElement.parseElement("6",null, parseInfo, null,null,1)) {
				...
				assertEquals("Invalid operator found.",6,parseInfo.getTreeElement().getValue().getRealValue(mc).doubleValue(),0);
			}
			else {
			...
			if (expressionElement.parseElement("6.2",null, parseInfo, null,null,1)) {
				...
				assertEquals("Invalid operator found.",6.2,parseInfo.getTreeElement().getValue().getRealValue(mc).doubleValue(),0);
			}
			else {
				fail ("Cannot parse a valid real value (1)");
			}
			...
			if (expressionElement.parseElement("6e-2",null, parseInfo, null,null,1)) {
				...
				assertEquals("Invalid operator found.",6e-2,parseInfo.getTreeElement().getValue().getRealValue(mc).doubleValue(),0);
			}
			else {
				fail ("Cannot parse a valid real value (1)");
			}
			...
			if (expressionElement.parseElement("6.2e-2",null, parseInfo, null,null,1)) {
				...
				assertEquals("Invalid operator found.",6.2e-2,parseInfo.getTreeElement().getValue().getRealValue(mc).doubleValue(),0);
			}
			else {
				fail ("Cannot parse a valid real value (1)");
			}
	...
	public void testSimpleExpressionExpressionElement() {
		try {
			...
			if (expressionElement.parseElement("6.2e-5",ExpressionModelFactory.getDefaultExpressionModel(), parseInfo, ExpressionFactory.getCatalog(),functionParameters,1)) {
				assertEquals("Invalid parse end position.",6,parseInfo.getEndPos());
				assertEquals("Invalid operator found.",6.2e-5,parseInfo.getTreeElement().getValue().getRealValue(mc).doubleValue(),0);
			}
			else {
				fail ("Cannot parse a valid simple expression value (1)");
			}
			...
			if (expressionElement.parseElement("6i",ExpressionModelFactory.getDefaultExpressionModel(), parseInfo, ExpressionFactory.getCatalog(),functionParameters,1)) {
				assertEquals("Invalid parse end position.",2,parseInfo.getEndPos());
				assertEquals("Invalid operator found.",6,parseInfo.getTreeElement().getValue().getComplexValue(mc).doubleValue(),0);
			}
			else {
				fail ("Cannot parse a valid simple expression value (1)");
			}
			...
			if (expressionElement.parseElement("6.2e-5i",ExpressionModelFactory.getDefaultExpressionModel(), parseInfo, ExpressionFactory.getCatalog(),functionParameters,1)) {
				assertEquals("Invalid parse end position.",7,parseInfo.getEndPos());
				assertEquals("Invalid operator found.",6.2e-5,parseInfo.getTreeElement().getValue().getComplexValue(mc).doubleValue(),0);
			}
			else {
				fail ("Cannot parse a valid simple expression value (1)");
			}
			...
			catalog.addConstant("const",new RealImpl(new BigDecimal(10.4)));
