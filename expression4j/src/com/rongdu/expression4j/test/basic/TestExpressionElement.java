...
package com.rongdu.expression4j.test.basic;
...
import com.rongdu.expression4j.basic.impl.BinaryOperatorExpressionElement;
import com.rongdu.expression4j.basic.impl.ComplexOrRealExpressionElement;
import com.rongdu.expression4j.basic.impl.ConstantOrVariableExpressionElement;
import com.rongdu.expression4j.basic.impl.ConstantOrVariableWithoutCheckExpressionElement;
import com.rongdu.expression4j.basic.impl.FunctionDefExpressionElement;
import com.rongdu.expression4j.basic.impl.FunctionExpressionElement;
import com.rongdu.expression4j.basic.impl.FunctionNameExpressionElement;
import com.rongdu.expression4j.basic.impl.IntegerExpressionElement;
import com.rongdu.expression4j.basic.impl.ParametersDefExpressionElement;
import com.rongdu.expression4j.basic.impl.ParametersExpressionElement;
import com.rongdu.expression4j.core.Catalog;
import com.rongdu.expression4j.core.ExpressionModel;
import com.rongdu.expression4j.core.exception.ModelException;
import com.rongdu.expression4j.core.exception.ParsingException;
import com.rongdu.expression4j.factory.ExpressionFactory;
import com.rongdu.expression4j.factory.ExpressionModelFactory;
import com.rongdu.expression4j.factory.OperatorFactory;
...
public class TestExpressionElement extends TestCase {
	...
	public void testComplexOrRealExpressionElement() {
		try {
			...
			assertEquals("Invalid value found.",2.34,parseInfo.getTreeElement().getValue().getRealValue(mc).doubleValue(),MathUtil.getZero().doubleValue());
			...
			assertEquals("Invalid parse end position.",6,parseInfo.getEndPos());
			assertEquals("Invalid type result found.",1,parseInfo.getTreeElement().getValue().getType());
			assertEquals("Invalid value found.",234,parseInfo.getTreeElement().getValue().getRealValue(mc).doubleValue(),MathUtil.getZero().doubleValue());
			...
			try {
				...
				assertEquals("Invalid real value found.",0,parseInfo.getTreeElement().getValue().getRealValue(mc).doubleValue(),MathUtil.getZero().doubleValue());
				assertEquals("Invalid value found.",2.34,parseInfo.getTreeElement().getValue().getComplexValue(mc).doubleValue(),MathUtil.getZero().doubleValue());
				...
				assertEquals("Invalid parse end position.",7,parseInfo.getEndPos());
				assertEquals("Invalid type result found.",2,parseInfo.getTreeElement().getValue().getType());
				assertEquals("Invalid real value found.",0,parseInfo.getTreeElement().getValue().getRealValue(mc).doubleValue(),0);
				assertEquals("Invalid value found.",234,parseInfo.getTreeElement().getValue().getComplexValue(mc).doubleValue(),0);
			}
	...
	public void testIntegerExpressionElement() {
		try {
			...
			if (expressionElement.parseElement("123",null, parseInfo, null,functionParameters,1)) {
				assertEquals("Invalid parse end position.",3,parseInfo.getEndPos());
				assertEquals("Invalid function found.",123,parseInfo.getTreeElement().getValue().getRealValue(mc).doubleValue(),0);
			}
			else {
			...
			if (expressionElement.parseElement("1234567890",null, parseInfo,ExpressionFactory.getCatalog(),functionParameters,1)) {
				...
				assertEquals("Invalid function found.",1234567890,parseInfo.getTreeElement().getValue().getRealValue(mc).doubleValue(),0);
			}
			else {
			...
			if (expressionElement.parseElement("1234.567890",null, parseInfo,ExpressionFactory.getCatalog(),functionParameters,1)) {
				assertEquals("Invalid parse end position.",4,parseInfo.getEndPos());
				assertEquals("Invalid function found.",1234,parseInfo.getTreeElement().getValue().getRealValue(mc).doubleValue(),0);
			}
			else {
				fail("Could not parse a valid integer 2.");
			}
