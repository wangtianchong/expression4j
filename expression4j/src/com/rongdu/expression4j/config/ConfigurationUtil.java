...
import com.rongdu.expression4j.config.BinaryOperator;
import com.rongdu.expression4j.config.CatalogDocument;
import com.rongdu.expression4j.config.Catalogs;
import com.rongdu.expression4j.config.ConfigurationDocument;
import com.rongdu.expression4j.config.Constant;
import com.rongdu.expression4j.config.ModelDocument;
import com.rongdu.expression4j.config.Models;
import com.rongdu.expression4j.config.OperatorDocument;
import com.rongdu.expression4j.config.OperatorManagerDocument;
import com.rongdu.expression4j.config.OperatorManagers;
import com.rongdu.expression4j.config.Operators;
import com.rongdu.expression4j.core.Catalog;
import com.rongdu.expression4j.core.ConfigurationException;
	...
	public static final void loadConfig(InputStream xmlConfigStream) throws ConfigurationException {
		try {
			...
			for (int i=0; i<modelList.length; i++) {
				...
				//add expression models
				log.debug("Add expression models");
				com.rongdu.expression4j.config.ExpressionElement[] expressionElementList = currentModelXML.getExpressionElementArray();
			...
			for (int i=0; i<catalogList.length; i++) {
				...
				Catalog currentCatalog = ExpressionFactory.createCatalog(currentCatalogName,MathUtil.getDefaultMathContext());
				...
				for (int j=0; j<constantList.length; j++) {
					...
					MathematicalElement currentConstant = new RealImpl(MathUtil.getZero());
				...
				com.rongdu.expression4j.config.Expression[] expressionList = catalogList[i].getExpressionArray();
		...
		try {
			...
			while (iter.hasNext()) {
				...
				for (int i=1; i<=nbExpressionElementPriority; i++) {
					...
					com.rongdu.expression4j.config.ExpressionElement expElemXML = currentModelXML.addNewExpressionElement();
			...
			while (iter.hasNext()) {
				...
				while (iter2.hasNext()) {
					...
					com.rongdu.expression4j.config.Constant constantXML = catalogXML.addNewConstant();
					...
					while (enm.hasMoreElements()) {
						...
						com.rongdu.expression4j.config.Constant.ConstantProperty currentConstantProperty = constantXML.addNewConstantProperty();
				...
				while (iter2.hasNext()) {
					...
					com.rongdu.expression4j.config.Expression expressionXML = catalogXML.addNewExpression();
