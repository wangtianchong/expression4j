...
public class OperatorPlusRealComplex implements OperatorImpl {
	...
	public MathematicalElement compute(MathContext mc,MathematicalElement leftElement,
		...
		try {
			...
			if (leftElement.getType() == 1 && rightElement.getType() == 2) {
				realValue = leftElement.getRealValue(mc).add(rightElement.getRealValue(mc), mc) ;
				...
				realValue = leftElement.getRealValue(mc).add(rightElement.getRealValue(mc),mc);
				complexValue = leftElement.getComplexValue(mc);
				return new ComplexImpl(realValue,complexValue);
			}
			else {
				throw new EvalException("Incosistent element type for operator plus real complex.");
			}
			
		}
		catch (MathematicalException me) {
			throw new EvalException("Cannot evaluate value for plus real complex.",me);
		}
	}
