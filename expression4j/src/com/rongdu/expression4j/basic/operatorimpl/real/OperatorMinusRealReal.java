...
public class OperatorMinusRealReal implements OperatorImpl {
	...
	public MathematicalElement compute(MathContext mc,MathematicalElement leftElement,
		...
		if (leftElement.getType() != 1 || rightElement.getType() != 1) {
			throw new EvalException("Incosistent element type for operator minus real real.");
		}
		
		return new RealImpl(leftElement.getRealValue(mc).subtract(rightElement.getRealValue(mc),mc));
	}
