...
public class OperatorDivideRealReal implements OperatorImpl {
	...
	public MathematicalElement compute(MathContext mc,MathematicalElement leftElement,
		...
		if (leftElement.getType() != 1 || rightElement.getType() != 1) {
			throw new EvalException("Incosistent element type for operator divide real real.");
		}
		
		return new RealImpl(leftElement.getRealValue(mc).divide(rightElement.getRealValue(mc),mc));
	}
